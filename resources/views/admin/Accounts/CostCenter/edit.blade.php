          ...
          <div class="col-12">
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">  {{ trans('Account.editacostcenter') }}     </h3>
              </div>
              <div class="card-body">
                <form class="user" method="POST" action="{{ route('costcenters.update',$Costcenteer->id) }}" enctype = "multipart/form-data">
                  @csrf
                  <div class="pl-lg-4">
                    <div class="row">
                      <div class="col-lg-6">
                        <div class="form-group">
                          ...
                          <input type="number"  class="form-control @error('CostCodeID') is-invalid @enderror" id="CostCodeID" value="{{$Costcenteer->CostCodeID}}" name="CostCodeID" placeholder="   {{ trans('Account.Code') }}   ">
                          @error('CostCodeID')
                      ...
                      <div class="col-lg-6">
                        <div class="form-group">
                          ...
                          <input type="text"  class="form-control @error('CostName') is-invalid @enderror" id="CostName"value="{{$Costcenteer->CostName}}" name="CostName" placeholder=" {{ trans('Account.Costname') }}   ">
                          @error('CostName')
                          <span class="invalid-feedback" role="alert">
                              <strong>{{ $message }}</strong>
                          </span>
                      ...
                      <div class="col-lg-6">
                        <div class="form-group">
                          ...
                          <input type="text"  class="form-control @error('CostNameEN') is-invalid @enderror" id="CostNameEN"value="{{$Costcenteer->CostNameEN}}" name="CostNameEN" placeholder="  {{ trans('Account.CostnameinEnglish') }}">
                          @error('CostNameEN')
                          <span class="invalid-feedback" role="alert">
                              <strong>{{ $message }}</strong>
                          </span>
                        ...
                        <div class="form-group">
                          ...
                          <select class="form-control @error('nameFather') is-invalid @enderror" id="nameFather"  onkeypress="return ValidateKey();" name="nameFather" placeholder="  {{ trans('Account.Mainaccount') }} ">
                            @foreach (auth()->user()->organization->CostCenter as $item)
                              <option @if ($Costcenteer->SourceID ==$item->id)  selected @endif value="{{$item->CostCodeID}}::{{ $item->id}}::{{ $item->CostName}}">{{ $item->CostName}}</option>
                            @endforeach
                        </select>
                        </div>
                      </div>
