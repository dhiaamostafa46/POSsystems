                ...
                <div class="col-12">
                    <div class="card ">
                        <div class="card-header  card-primary">
                            ...
                            <button type="button" class="btn btn-primary floatmleft" data-toggle="modal"
                                data-target="#createnodeltask">
                                {{ trans('HR.Add') }}
                            </button>
                            <br>
                        </div>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ route('Task.show', $asset->id) }}"
                                                        ...
                                                        {{ trans('HR.show') }}</a>
                                                    <button type="button" class="btn btn-primary" onclick="showModal({{$asset->id}})">
                                                        {{ trans('HR.addcomment') }}
                                                    </button>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" action="{{ route('Task.update', 1) }}" enctype = "multipart/form-data">
                    @method('PUT')
                    @csrf
...
<div class="modal fade" id="createnodeltask" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">
            ...
            <div class="modal-body">
                <form class="user" method="POST" action="{{ route('Task.store') }}" enctype = "multipart/form-data">
                    @csrf
                    <div class="pl-lg-4">
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="form-group">
                                    <label class="form-control-label" for="input-username">
                                        {{ trans('HR.typetask') }}</label>
                                    <select class="form-control" id="typetask" name="typetask"
                                        onchange="funtypetask()">
                                        <option value="1"> {{ trans('HR.departments') }} </option>
                                        <option value="0"> {{ trans('HR.employees') }} </option>
                                    </select>

                                </div>
                            </div>
                            <div class="col-lg-12">
                                <div class="form-group" style="display: none" id="groupempcreate">
                                    <label class="form-control-label" for="input-username">
                                        {{ trans('HR.EmployeeName') }}</label>
                                    <select class="form-control" id="emp" name="empcreate">
                                        @foreach (auth()->user()->organization->Employee->where('Status' ,1) as $item)
                                            <option value="{{ $item->id }}"> {{ $item->nameAr }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group " id="groupdepartments">
                                    <label class="form-control-label" for="input-username">
                                        {{ trans('HR.departments') }}</label>
