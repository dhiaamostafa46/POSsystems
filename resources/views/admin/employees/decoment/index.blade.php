...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0"> {{ trans('HR.typedocuments') }}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb floatmleft">
                        <li class="breadcrumb-item"><a href="#">{{ trans('HR.employees') }}</a></li>
                        <li class="breadcrumb-item active"> {{ trans('HR.typedocuments') }}</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h6 style="display: inline-block"> {{ trans('HR.typedocuments') }}</h6>
                                    ...
                                    @if (count($doc) > 0)
                                        @foreach ($doc as $index => $type)
                                            <tr>
                                                ...
                                                <td>
                                                    <a data-toggle="modal"
                                                        data-target="#exampleModal{{ $index }}"class="btn btn-warning"><i
                                                            class="fa fa-edit"></i>
                                                        {{ trans('HR.Edit') }}</a>
                                                        ...
                                                        <div class="modal-dialog" role="document">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title" id="exampleModalLabel">  تعديل نوع المستند
                                                                         </h5>

                                                                </div>
                                                                <div class="modal-body">
                                                                    <form class="user" method="POST"
                                                                        action="{{ route('Decmentype.update', $type->id) }}"
    ...
    <div class="modal fade modal" id="CreateModal" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title text-center" id="exampleModalLabel"> {{ trans('HR.typedocuments') }} </h5>
                </div>
                <div class="modal-body">
                    <form class="user" method="POST" action="{{ route('Decmentype.store') }}"
                        enctype = "multipart/form-data">
                        @csrf

                        <div class="form-group">
                            <label for="exampleInputEmail1">  {{ trans('HR.NameArabic') }} </label>
                            <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" name="nameAr" required
                               >

                        </div>
                        <div class="form-group">
                            <label for="exampleInputPassword1">  {{ trans('HR.NameEnglish') }}</label>
                            <input type="text" class="form-control" id="exampleInputPassword1" name="nameEn" required
                               >
                        </div>
