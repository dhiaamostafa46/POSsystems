        ...
        <div class="row mt-2">
                  ...
                  <div class="pl-lg-4">
                    <div class="row">
                      <div class="col-lg-12">
                        <div class="form-group">
                          <h1>
                            <strong id="bigtotal">0</strong> <span class="text-small">{{ trans('Sale.Rial') }}</span>
                          </h1>
                        </div>
                      </div>
                    </div>
          ...
          <div class="col-12">
            <div class="card card-primary" style="height: 900px">
              ...
              <div class="card-body">
                <form class="user" method="POST" action="{{route('OfferPrice.update',$order->id)}}"  target="_blank" enctype = "multipart/form-data" onsubmit="setTimeout(function(){window.location.reload();},10);">
                  @csrf
                ...
                <div class="row">
                  <div class="col-lg-8">
                    <div class="row">
                            ...
                            <div class="form-group">
                              <select class="form-control @error('customerID') is-invalid @enderror" id="customerID" name="customerID" onchange="addCustomer(this)">
                                ...
                                @foreach (auth()->user()->organization->customers as $customer)
                                    <option @if ($customer->id ==$order->customerID) selected  @endif value="{{$customer->id}}">{{$customer->name}}</option>
                                @endforeach
                              </select>
                              @error('customerID')
                       ...
                       <div class="col-lg-2">
                            <div class="form-group row">
                                <input type="number"  value="{{$order->Duration}}"  class="form-control col-9"  id="Duration" name="Duration" placeholder=" {{ trans('Sale.validityOfferPrice') }} ">
                    ...
                    <div class="row">
                      <input type="hidden" name="total" id="total" value="0"  value="{{$order->totalwvat}}" />
                      <input type="hidden" name="vat" id="vat"  value="{{$order->totalvat}}" />
                      <input type="hidden" name="totalwvat" id="totalwvat" value="{{$order->totalvat+$order->totalwva}}" />
...
$( document ).ready(function() {
          ...
          data ="";
          count=0;
            @foreach( $order->OfferPricedetails as $index => $product )
                count=count+1;
                data = data+'<tr id="tr-{{$index+1}}-"> <td>{{$index+1}}</td> <td>{{$product->productName}}</td>';
                data = data+' <td><input type="number" min="0.00" step=".01"  class="form-control text-center" id="price{{$index+1}}" name="price{{$index+1}}" value="{{$product->price}}" onchange="calculate({{$index+1}})" readonly></td>';
                data = data+'<td><input type="number" min="0.00" step=".01"  class="form-control text-center" id="quantity{{$index+1}}" name="quantity{{$index+1}}" value="{{$product->quantity}}" onchange="calculate({{$index+1}})"></td>';
                data = data+' <td><input type="number" min="0.00" step="0.01"  class="form-control text-center" id="Profit{{$index+1}}" name="Profit{{$index+1}}" value="{{$product->Profit}}" onchange="calculate({{$index+1}})"></td>';
                data = data+'<td><input type="number" min="0.00" step="0.01"  class="form-control text-center"  id="deduct{{$index+1}}" name="deduct{{$index+1}}" value="{{$product->discount}}" onchange="calculate({{$index+1}})"></td>';
                ...
                data = data+' <input type="hidden" name="cprice{{$index+1}}" id="cprice{{$index+1}}" value="{{$product->totalcost}}" />';
                data = data+'   <input type="hidden" name="itemName{{$index+1}}" id="itemName{{$index+1}}" value="{{$product->productName}}" />';
                ...
                data = data+'   <input type="hidden" name="vatValue{{$index+1}}" id="vatValue{{$index+1}}" value="15" />';
            ...
            $('#view-total').append("111");
            $('#view-vat').append("1felf");
            $('#view-totalwvat').append("weeeeeee");
