    ...
    <script>
        $(document).ready(function() {
            ...
            @foreach ($order->orderdetails as $index => $product)
                ...
                data = data +
                    '<td><input type="number" min="0.00" step="0.01"  class="form-control text-center"  id="discountval{{ $index + 1 }}" name="discountval{{ $index + 1 }}" value="{{ $product->discount }}" onchange="calculatediscount({{ $index + 1 }})"></td>';
                data = data +
                ...
                success: data => {
                    ...
                    if (data.flage == 0) {
                        ...
                        data.items.forEach(item => {
                            ...
                            <td><input type="number" min="0.00" step=".01"  class="form-control text-center"  id="discountval${count}" name="discountval${count}" value="${data.discount}" onchange="calculatediscount(${count})"></td>
