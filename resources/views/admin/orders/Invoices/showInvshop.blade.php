            ...
            <div class="cs-invoice_in" id="download_section">
                <div class="cs-invoice_head cs-type1 cs-mb25">
                    ...
                    <div class="text-center">
                        ...
                        <p class="cs-invoice_date cs-primary_color cs-m0"><b class="cs-primary_color">
                                @if ($order->TypeInv == 2)
                                    {{ trans('Sale.invoicesNumber') }} :
                            </b>{{ $order->serial }}
                            @endif
                        </p>
                        <p class="cs-invoice_date cs-primary_color cs-m0"><b
                                class="cs-primary_color">{{ trans('Sale.Date') }} : </b>{{ $order->created_at }} </p>
                    </div>
                ...
                <div class="cs-invoice_head cs-mb10">
                    ...
                    <div class="cs-invoice_right cs-text_right">
                        ...
                        <p>
                            <span> {{ trans('Sale.customername') }} : @if ($order->FlageCustumer == -1)
                                    {{ $order->VirtualCustomer->name ?? '' }}
                                @else
                                    {{ $order->Customer->name ?? '' }}
                                @endif </span> <br>
                            <span> {{ trans('Sale.Customertaxnumber') }} :
                            ...
                            <span> {{ trans('Sale.address') }} :

                                @if ($order->FlageCustumer == -1)
                                    {{ $order->VirtualCustomer->addressAr ?? '' }}
                                @else
                                    {{ $order->Customer->addressAr ?? '' }}
                                @endif
                            </span> <br>
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        @if ($order->orderType == 4)
                                            <th class=" cs-semi_bold cs-primary_color cs-focus_bg"> خصم %</th>
                                            <th class=" cs-semi_bold cs-primary_color cs-focus_bg"> خصم بالسعر</th>
                                        @endif
                                        <th class=" cs-semi_bold cs-primary_color cs-focus_bg">الوصف</th>
                                        <th class=" cs-semi_bold cs-primary_color cs-focus_bg">الاجمالي غير شامل ضريبة ق.م
                                        </th>
                                        <th class=" cs-semi_bold cs-primary_color cs-focus_bg">اجمالي ضريبة ق.م 15% </th>
                                        <th class=" cs-semi_bold cs-primary_color cs-focus_bg cs-text_right">الاجمالي شامل
                                            ضريبة ق.م</th>
                                    </tr>
                                    <tr>
                                        ...
                                        @if ($order->orderType == 4)
                                            <th class=" cs-semi_bold cs-primary_color cs-focus_bg"> descount %</th>
                                            <th class=" cs-semi_bold cs-primary_color cs-focus_bg"> descount price </th>
                                        @endif
                                        <th class=" cs-semi_bold cs-primary_color cs-focus_bg">description</th>
                                    ...
                                    @if (count($order->orderdetails) > 0)
                                        @foreach ($order->orderdetails as $item)
                                            <tr>
                                                ...
                                                @endif
                                                <td style="font-size: large">{{ $item->desc }}</td>
                                                <td style="font-size: large">{{ number_format($item->total, 2) }}</td>
                                                <td style="font-size: large">
                                                    {{ number_format($item->total * 0.15, 2) }}</td>
                                                <td style="font-size: large">
                                                    {{ number_format($item->total + $item->total * 0.15, 2) }}
                                                </td>
                                            </tr>
                                        @endforeach
                                    @endif
                                    ...
                                    <tbody>
                                        <tr class="cs-border_left">
                                            ...
                                            <td class="cs-width_2 cs-semi_bold cs-focus_bg cs-primary_color cs-text_right">
                                                {{ number_format($order->totalwvat - $order->totalvat, 2) }}
                                                {{ trans('Sale.Rial') }} </td>
                                        </tr>
                                        <tr class="cs-border_left">
                                            <td class="cs-width_2 cs-semi_bold cs-primary_color cs-focus_bg">
                                            ...
                                            <td class="cs-width_2 cs-semi_bold cs-focus_bg cs-primary_color cs-text_right">
                                                {{ number_format($order->totalvat, 2) }} {{ trans('Sale.Rial') }} </td>
                                        </tr>
                                        @if ($order->ispaied != 0)
                                            <tr class="cs-border_left">
                                                <td class="cs-width_2 cs-semi_bold cs-primary_color cs-focus_bg">
                                                    {{ trans('Sale.Discount') }}
                                                    {{ number_format($order->ispaied, 2) }} %</td>
                                                <td
