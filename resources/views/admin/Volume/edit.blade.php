        ...
        <div class="row mt-2">
          ...
          <div class="col-12">
            <div class="card card-primary" style="height: 670px">
              ...
              <div class="card-body">
                <form class="user" method="POST" action="{{route('Volume.update' ,$Volume->id)}}" enctype = "multipart/form-data">
                  @csrf
    ...
    $( document ).ready(function() {
            ...
            @if ($Flage==1)
                    @foreach( $Volume->VolumeDetail as $index => $product )
                        ...
                        data = data+'<td>{{$product->coststore}}</td>';
                        data = data+'<td id="totalitems{{$index+1}}"">{{(float)$product->coststore * (float)$product->Quantity}} </td>';

                        data = data+'   <td class="text-center"><input type="hidden" name="item{{$index+1}}" id="item{{$index+1}}" value="{{$product->ProdectId}}"/>';
                        data = data+'<input type="hidden" name="unitid{{$index+1}}" id="unitid{{$index+1}}" value="{{$product->unitID}}" />';

                        data = data+'    <input type="hidden" name="CostStore{{$index+1}}" id="CostStore{{$index+1}}" value="{{$product->coststore}}" />';
                        ...
                        data = data+'   <input type="hidden"  step="any" name="rtotalwvat{{$index+1}}" id="rtotalwvat{{$index+1}}" value="{{$product->total}}" />';
