          ...
          <div class="col-12">
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">{{ trans('Sandat.EditExpenseitem') }}</h3>
              </div>
              <div class="card-body">
                <form class="user" method="POST" action="{{ route('outcomeCategories.update',$outcomecategory->id) }}" enctype = "multipart/form-data">
                  @csrf
                  @method('PUT')
                  <input type="hidden" value="{{$outcomecategory->AccountID}}" name="AccountID">
                  <div class="pl-lg-4">
                    <div class="row">
                      <div class="col-lg-6">
                        <div class="form-group">
                          <label class="form-control-label" for="input-username">{{ trans('Sandat.ItemnameArabic') }}</label>
                          <input type="text"  class="form-control @error('nameAr') is-invalid @enderror" id="nameAr" name="nameAr" placeholder="{{ trans('Sandat.ItemnameArabic') }}" value="{{$outcomecategory->nameAr}}">
                          @error('nameAr')
                      ...
                      <div class="col-lg-6">
                        <div class="form-group">
                          ...
                          <input type="text"  class="form-control @error('nameEn') is-invalid @enderror" id="nameEn"  onkeypress="return ValidateKey();" name="nameEn" placeholder="{{ trans('Sandat.ItemnameEnglish') }} " value="{{$outcomecategory->nameEn}}">
                          @error('nameEn')
                          <span class="invalid-feedback" role="alert">
                              <strong>{{ $message }}</strong>
                          </span>
                      ...
                      <div class="col-lg-6">
                        <div class="form-group">
                          ...
                          <select class="form-control @error('TypeAccount') is-invalid @enderror" id="TypeAccount" name="TypeAccount" placeholder=" {{ trans('Sandat.Itemtype') }}">
                            ...
                            @if ($item->AccountName == $outcomecategory->TypeAccount)
                               <option  value="{{$item->AccountID}}::{{$item->AccountName}}" selected>{{$item->AccountName}}</option>
                            @else
