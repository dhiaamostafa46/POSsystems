...
<script>
    $( document ).ready(function() {
              ...
              countval=0;
                    ...
                    @foreach( $Volume->Manufacturdetials as $index => $product )
                    countval=countval+1;
                        costprodect='{{(float)$product->product->unitprodect->where("StoreId",$product->storeid)->where("id",$product->unitID)->first()->costprodect ?? 0 }}';
                        count ='{{(float)$product->product->unitprodect->where("StoreId",$product->storeid)->where("id",$product->unitID)->first()->count ?? 0 }}';
                        start ='{{(float)$product->product->unitprodect->where("StoreId",$product->storeid)->where("id",$product->unitID)->first()->start ?? 0}}';
                        comeIn= '{{(float)$product->product->unitprodect->where("StoreId",$product->storeid)->where("id",$product->unitID)->first()->comeIn ?? 0}}';
                        saller ='{{(float)$product->product->unitprodect->where("StoreId",$product->storeid)->where("id",$product->unitID)->first()->saller ?? 0 }}';
                        Tainted ='{{(float)$product->product->unitprodect->where("StoreId",$product->storeid)->where("id",$product->unitID)->first()->Tainted ?? 0}}';
                        ComeOut= '{{(float)$product->product->unitprodect->where("StoreId",$product->storeid)->where("id",$product->unitID)->first()->ComeOut ?? 0}}';
                        hang= '{{(float)$product->product->unitprodect->where("StoreId",$product->storeid)->where("id",$product->unitID)->first()->hang ?? 0}}';
                        ...
                        data = data+'<td id="totalitems{{$index+1}}"">{{(float)($product->product->unitprodect->where("StoreId",$product->storeid)->where("id",$product->unitID)->first()->costprodect ?? 0) * (float)$product->Quantity}} </td>';
                        ...
                        data = data+'   <input type="hidden"  step="any" name="rtotalwvat{{$index+1}}" id="rtotalwvat{{$index+1}}" value="{{(float)($product->product->unitprodect->where("StoreId",$product->storeid)->where("id",$product->unitID)->first()->costprodect ?? 0) * (float)$product->Quantity}} " />';
            ...
            if(data.flage ==0)
            {
                ...
                data.items.forEach(item =>
                    ...
                    <tr id="tr-${count}-">
                        ...
                        <td><input type="number"  class="form-control text-center"  step="any" min="0.00"   id="bookCount${count}"  name="bookCount${count}" value="1" readonly ></td>
