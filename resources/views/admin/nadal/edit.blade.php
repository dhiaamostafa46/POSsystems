...
<section class="content">
  <div class="container-fluid">
    ...
    <div class="row mt-2">
                    ...
                    @foreach (auth()->user()->organization->prodcategoriesKatcSaller as $index => $cat)
                        ...
                        <div class="row">
                            ...
                            <a href="#" onclick="addItem('{{$item->id}}','{{$item->nameAr}}','{{$item->prodPrice}}','{{$item->costPrice}}','{{$item->kitchenID}}');" class="card shadow">
                                ...
                                <img src="{{asset('dist/img/products/'.$item->img)}}" height="100px" width="100%" alt="">
                                </div>
      ...
      <form action="{{route('Nadal.updatenadel',$orders->id)}}" method="POST" class="col-lg-3" >
        {{-- @method('PUT') --}}
        @csrf
        <div class="card pos-container" style="height: 800px">
          ...
          <div class="card-footer">
            <div  id="tr-${count}-">
              ...
              <input type="hidden" name="totalwvatold" id="totalwvatold" value="{{$orders->totalwvat}}" />
              <input type="hidden" name="totaldiscount" id="totaldiscount" value="0" />
              <input type="hidden" name="count" id="count" value="0">
              <input type="hidden" name="countold" id="countold" value="0">
        ...
        <select class="form-control"id="tblNo" name="tblNo"   style="width: 100%;display:inline-block" required>
            ...
            @if (count($table) >0)
               ...
               <option value="{{$item->id}}" @if ($item->id == $orders->tblNo ) @selected(true) @endif> {{$item->tableNo}} </option>
               @endforeach
            @endif
        </select>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title text-center" id="exampleModalLabel">  {{ trans('Sale.Ingredients') }} </h5>
...
<script>
    ...
    $( document ).ready(function() {
              ...
              data ="";
              count=0;
                @foreach( $orders->orderdetails as $index => $product )
                   count=count+1;

                   data =data +`<div  id="tr-{{$index+1}}-">
                                <h6 class="text-menu row">
                                <span class="col-4" style="font-size:small" id="name{{$index+1}}">{{$product->productName}}</span>
                                <span class="col-4 row">

                                    <input type="number" class="form-control col-8" name="quantity{{$index+1}}" id="quantity{{$index+1}}" value="{{$product->quantity}}" style="padding:1px;text-align:center" readonly  onchange="changeitemscount({{$index+1}})">
                                    <a class"btn btn-success col-2" onclick="qntyMinus({{$index+1}})"><i class="fa fa-minus-square" style="line-height:2"></i></a>
                                </span>
                                <span class="col-3"><input type="text" class="form-control" id="rtotalwvat{{$index+1}}" value="{{$product->total}}" readonly></span>
                                <span class="col-1"> <a class"btn btn-success" onclick="Extra({{$index+1}})"><i class="fa fa-list-ol" style="line-height:2"></i></a></span>
                                </h6>
                                <h6 class="text-menu row" id="menuExt{{$index+1}}">
                                </h6>
                                    <input type="hidden" name="item{{$index+1}}" id="item{{$index+1}}" value="" />
                                    <input type="hidden" name="itemcode{{$index+1}}" id="itemcode{{$index+1}}" value="{{$product->productID}}" />
                                    <input type="hidden" name="flagcode{{$index+1}}" id="flagcode{{$index+1}}" value="1" />
                                    <input type="hidden" name="kitchenID{{$index+1}}" id="kitchenID{{$index+1}}" value="{{$product->kitchenID}}" />
                                    ...
                                    <input type="hidden" name="price{{$index+1}}" id="price{{$index+1}}" value="{{$product->price}}" />
                                    <input type="hidden" name="cprice{{$index+1}}" id="cprice{{$index+1}}" value="{{$product->product->costPrice ??''}}" />
                                    <input type="hidden" name="Exitetprice{{$index+1}}" id="Exitetprice{{$index+1}}" value="0" />
                                    <input type="hidden" name="Exitetcount{{$index+1}}" id="Exitetcount{{$index+1}}" value="0" />
                                <hr>
                            </div>`;
                ...
                document.getElementById('countold').value = count;
                calculateTotal();
    ...
    function addProdectAll(id,name,price,cprice,kitchenID)
    {
        ...
        //****************************************************************
          ...
          $('#items').append(`
            ...
            <input type="hidden" name="itemcode${count}" id="itemcode${count}" value="${id}" />
            <input type="hidden" name="flagcode${count}" id="flagcode${count}" value="0" />
...
function removeItem(trnum){
    ...
    myobj.remove();
    rows--;
    // document.getElementById("tr-"+trnum+"-").style.display = "none";
    calculateTotal();
  }
