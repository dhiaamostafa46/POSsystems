            ...
            <div class="row mt-2">
                ...
                <div class="col-12">
                    <div class="card card-primary" style="height: 700px">
                        ...
                        <div class="card-body">
                            <form class="user" method="POST" action="{{ route('credorders.store') }}"
                                        ...
                                        <div class="row">
                                            <div class="col-lg-2">
                                                <div class="form-group">
                                                    <input type="text"
                                                        ...
                                                        value="{{ $Purchase->serial }}" id="serial" readonly
                                                ...
                                                <div class="form-group">
                                                    <select class="form-control @error('customerID') is-invalid @enderror"
                                                        ...
                                                        @foreach (auth()->user()->organization->customers as $customer)
                                                            <option @if ($Purchase->customerID == $customer->id) selected @endif
                                            ...
                                            <div class="col-lg-2">
                                                <div class="form-group">
                                                    ...
                                                    onchange="ShowDecscontall()" value="{{$Purchase->ispaied}}"  name="Decscontall" placeholder="خصم">
                                                </div>
                                            </div>
                                            <div class="col-lg-2">
                                                <div class="form-group">
                                                    <select class="form-control @error('type') is-invalid @enderror"
                                                        id="typeSale" onchange="showCashCard(this)" name="type">
                                            ...
                                            <div class="col-lg-3 row mb-3" id="paymentType"style="display: none">
        ...
        $(document).ready(function() {
            ...
            @if ($flage == 0)
                @foreach ($Purchase->orderdetails as $index => $product)
                    ...
                    data = data +
                        ' <td><input type="number" step="any"  class="form-control text-center" id="price{{ $index + 1 }}" name="price{{ $index + 1 }}" value="{{ $product->price }}" onchange="calculate({{ $index + 1 }})"></td>';
                    data = data +
                        '<td><input type="number"  step="any"  class="form-control text-center" id="quantity{{ $index + 1 }}" name="quantity{{ $index + 1 }}" value="{{ $product->quantity }}" onchange="calculate({{ $index + 1 }})"></td>';
                    data = data +
            ...
            @else
                @foreach ($Purchase->orderdetails as $index => $product)
                    ...
                    data = data +
                        '<tr id="tr-{{ $index + 1 }}-"> <td>{{ $index + 1 }}</td> <td>{{ $product->product->nameAr }}</td>';
                    data = data +
                        ' <td><input type="number"   class="form-control text-center" id="price{{ $index + 1 }}" name="price{{ $index + 1 }}" value="{{ $product->price }}" onchange="calculate({{ $index + 1 }})"></td>';
                    data = data +
                        '<td><input type="number"    class="form-control text-center" id="quantity{{ $index + 1 }}" name="quantity{{ $index + 1 }}" value="{{ $product->quantity }}" onchange="calculate({{ $index + 1 }})"></td>';
                    data = data +
                        '<td><input type="text"   class="form-control text-center"  id="desc{{ $index + 1 }}" name="desc{{ $index + 1 }}"  style="width: 90%"></td>';
    ...
    <script>
        ...
        $('#barcode').change(function() {
            ...
            $.ajax({
              ...
              <tr id="tr-${count}-">
                <td>${count}</td>
                <td>${item.nameAr}</td>
                <td><input type="number" step="any"  class="form-control text-center" id="price${count}" name="price${count}" value="${item.prodPrice}" onchange="calculate(${count})"></td>>
                ...
                <td><input type="text" class="form-control text-center" id="desc${count}" name="desc${count}"  style="width: 90%"></td>
        ...
        function calculateTotal() {
            ...
            if (rows > 0) {
                ...
                document.getElementById("vat").value = parseFloat(total * 0.15).toFixed(2);
                $("#view-vat").empty().append(document.getElementById('vat').value);

                document.getElementById("totalwvat").value = parseFloat(total + (total * 0.15)).toFixed(2);
