        ...
        <div class="row mt-2">
          ...
          <div class="col-12">
            <div class="card card-primary">
              ...
              <div class="card-body">
                <form class="user" method="POST" action="{{route('purchases.updateedit',$purchase->id)}}" enctype = "multipart/form-data">
                  ...
                  <div class="row">
                      ...
                      <div class="row">
                          ...
                          <div class="form-group">
                            ...
                            <select class="form-control @error('supplierID') is-invalid @enderror" id="supplierID" name="supplierID">
                              ...
                              @foreach (auth()->user()->organization->suppliers as $supplier)
                                  <option @if ($purchase->supplierID ==  $supplier->id) selected @endif value="{{$supplier->id}}::{{$supplier->AccountID}}">{{$supplier->name}}</option>
                              @endforeach
                            </select>
                            @error('supplierID')
                        ...
                        <div class="col-lg-3">
                          <div class="form-group">
                            ...
                            <select class="form-control @error('type') is-invalid @enderror" id="type" name="type" onchange="showCashCard(this)">
                              <option @if ($purchase->type ==  "121") selected @endif  value="121">{{ trans('purchases.Cash') }}</option>
                              <option @if ($purchase->type ==   "122") selected @endif value="122">{{ trans('purchases.Net') }}</option>
                              <option  @if ($purchase->type ==  "3") selected @endif value="3">{{ trans('purchases.Paylater') }}</option>
                            </select>
                            @error('type')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                        ...
                        <div class="col-lg-3" id="paymentDate" style="display: none">
                          <div class="form-group">
                            ...
                            <input type="date" class="form-control @error('payDate') is-invalid @enderror"   value="{{$purchase->payDate }}" id="payDate" name="payDate" placeholder=" {{ trans('purchases.Invoicedate') }}">
                            @error('payDate')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                        ...
                        <div class="col-lg-3">
                          <div class="form-group">
                            <label for=""> {{ trans('purchases.Bookinvoicenumber') }}</label>
                            <input type="text" class="form-control @error('serial') is-invalid @enderror" value="{{$purchase->serial }}" id="serial" name="serial" placeholder="{{ trans('purchases.Bookinvoicenumber') }}">
                            @error('serial')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                        ...
                        <div class="col-lg-3">
                          <div class="form-group">
                            <label for=""> {{ trans('purchases.Invoicedate') }}</label>
                            <input type="date" class="form-control @error('invoiceDate') is-invalid @enderror" value="{{$purchase->invoiceDate }}" id="invoiceDate" name="invoiceDate" placeholder="{{ trans('purchases.Invoicedate') }} ">
                            @error('invoiceDate')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                        ...
                        <div class="col-lg-3">
                            <div class="form-group">
                                ...
                                onchange="ShowDecscontall()" name="Decscontall"  value="{{$purchase->discount}}" placeholder="خصم">
                            </div>
                        </div>
                        ...
                        <div class="col-lg-3">
                          <div class="form-group">
                            ...
                            <select class="form-control @error('customerID') is-invalid @enderror" id="TypeFatorah" name="TypeFatorah" >
                              ...
                              <option value="2">  {{ trans('purchases.certified') }}  </option>
                            </select>
                          </div>
                        </div>
                        <div class="col-lg-4">
                    ...
                    <div class="col-lg-4">
                        <table class="table table-borderless">
                            <tr>
                              <th>  <h6>{{ trans('purchases.Total') }}</h6></th>
                              <th>   <h6 > <strong id="view-total"> </strong> </h6></th>
                            </tr>
                            <tr>
                              <td>  <h6>  {{ trans('purchases.Tax') }}</h6> </td>
                              <td>  <h6> <strong id="view-vat"> </strong></h6> </td>
                            </tr>
                            <tr>
                                <td> <h6>  {{ trans('purchases.Totalincludingtax') }}</h6> </td>
              ...
              county=0;
                ...
                @foreach( $purchase->purchasedetails as $index => $product )
                    ...
                    @foreach ($product->product->unitprodect as $i=> $items)
                      dataaa=dataaa+ `<option  @if ($product->idxUnit ==  $i) selected @endif   value="{{$items->id}}::{{$i}}::{{$items->costprodect}}" >{{$items->unitname}}</option> `;
                    @endforeach
                    county=county+1;
                    data = data+'<tr id="tr-{{$index+1}}-"> <td>{{$index+1}}</td> <td>{{$product->product->nameAr ??''}}</td>';
                    data = data+' <td class="px-2"><input type="number"  class="form-control text-center"  min="0.00" step=".01" id="price{{$index+1}}" name="price{{$index+1}}" value="{{number_format($product->price,2)}}" onchange="calculate({{$index+1}})" ></td>';
                    data = data+'<td class="px-2"><input type="number"  class="form-control text-center" min="0.00" step=".01"  id="quantity{{$index+1}}" name="quantity{{$index+1}}" value="{{$product->quantity}}" onchange="calculate({{$index+1}})"></td>';
                    data = data+'<td class="px-2" ><select  class="form-control text-center" onchange="doSomething({{$index+1}});" id="unit{{$index+1}}" required name="unit{{$index+1}}"> '+dataaa+' </select></td>';
                    data = data+' <td id="itotal{{$index+1}}">{{$product->price * $product->quantity}}</td>';
                    ...
                    data = data+'   <input type="hidden" name="rtotalwvat{{$index+1}}" id="rtotalwvat{{$index+1}}" value="{{$product->price * $product->quantity}}" />';
                    ...
                    icon: "warning"
                          ...
                          cost=0;
                            data.unit.forEach((item ,index)=>{
                                ...
                                dataaa=dataaa+ `<option  value="${item.id}::${index}::${item.costprodect}" >${item.unitname} </option> `;
                            })
