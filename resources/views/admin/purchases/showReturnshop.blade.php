  ...
  <div class="row col-12"  style="margin: auto">
    ...
    <table class="col-12 mt-3 border for-print" style="margin:auto;float:none;text-align:center;padding:5px" >
      ...
      <tr>
          ...
          <td style="font-size:small">{{number_format(($item->price * $item->quantity)*0.15,2)}}</td>
          <td style="font-size:small">{{number_format(($item->price * $item->quantity)+(($item->price * $item->quantity)*0.15),2)}}</td>
      </tr>
      @endforeach
      @endif
      <hr>
      ...
      <tfoot>
        ...
        <tr>
          <td colspan="5" style="background-color: rgb(202, 199, 199);font-size:small;height:30px">    الخصم   {{ number_format($purchase->discount, 2) }} %   Total VAT   </td>
