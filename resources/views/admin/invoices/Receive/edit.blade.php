          ...
          <div class="col-12">
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">  {{ trans('Sandat.EditSinadatindexReceive') }}  </h3>
              </div>
              <div class="card-body">
                <form class="user" method="POST" action="{{ route('Sinadat.update' ,$invoice->id) }}" enctype = "multipart/form-data">
                  @csrf
                  <div class="pl-lg-4">
                    <div class="row">
                      <div class="col-lg-3">
                        <div class="form-group">
                          <label class="form-control-label" for="input-username">{{ trans('Sandat.customername') }} </label>
                          ...
                          <select class="form-control @error('customerID') is-invalid @enderror" id="customerID" name="customerID">
                            <option value=""> {{ trans('Sandat.CustomerChoose') }} </option>
                            @foreach (auth()->user()->organization->customers as $customer)
                                 @if ($customer->id ==$invoice->customerID)
                                   <option selected value="{{$customer->id}}::{{$customer->AccountID}}" >{{$customer->name}}</option>
                                 @else
                                   <option value="{{$customer->id}}::{{$customer->AccountID}}" >{{$customer->name}}</option>
                                 @endif
                      ...
                      <div class="col-lg-3">
                        <div class="form-group">
                          ...
                          <input type="date" class="form-control @error('total') is-invalid @enderror" value="{{$invoice->date}}" id="date" name="date" >
                          @error('date')
                          <span class="invalid-feedback" role="alert">
                              <strong>{{ $message }}</strong>
                          </span>
                      ...
                      <div class="col-lg-3">
                        <div class="form-group">
                          ...
                          <input type="number" class="form-control @error('total') is-invalid @enderror" value="{{$invoice->total}}"  id="total" name="total" onchange="getRest();" placeholder="{{ trans('Sandat.Total') }}">
                          <input type="hidden"  name="Oldtotal" value="{{$invoice->total}}">
                          @error('total')
                          <span class="invalid-feedback" role="alert">
                              <strong>{{ $message }}</strong>
                          </span>
                      ...
                      <div class="col-lg-3">
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control @error('Ref') is-invalid @enderror" value="{{$invoice->invoicesID}}" id="Ref" name="Ref" value="" >
                          @error('Ref')
                          <span class="invalid-feedback" role="alert">
                              <strong>{{ $message }}</strong>
                          </span>
                      ...
                      <div class="col-lg-6">
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control text-right @error('comment') is-invalid @enderror"value="{{$invoice->comment}}" id="comment" name="comment" placeholder="{{ trans('Sandat.comment') }}">
                          @error('comment')
                          <span class="invalid-feedback" role="alert">
                              <strong>{{ $message }}</strong>
                          </span>
                      ...
                      <div class="col-lg-3">
                        <div class="form-group">
                          ...
                          <select class="form-control @error('paymentType') is-invalid @enderror" id="paymentType" name="paymentType"  onchange="payments()">
                            ...
                            @if ($invoice->paymentType =="121")
                              <option selected value="121">{{ trans('Sandat.Cash') }}</option>
                              <option value="122">{{ trans('Sandat.Net') }}</option>
                            @else
                                <option  value="121">{{ trans('Sandat.Cash') }}</option>
                                <option selected value="122">{{ trans('Sandat.Net') }}</option>
                            @endif
                      ...
                      <div class="col-lg-3" id="paymentAccount">
                        <div class="form-group">
                          ...
                          <select class="form-control @error('paymentTypeitems') is-invalid @enderror" id="paymentTypeitems" name="paymentTypeitems" >
                             <option value="{{$invoice->AccountID}}::{{$invoice->nameAccount}}">{{$invoice->nameAccount}}</option>
                          </select>
                        ...
                        <div class="form-group">
                          ...
                          <select class="form-control @error('CostCenter') is-invalid @enderror" id="CostCenter" name="CostCenter">
                            ...
                            @foreach (auth()->user()->organization->CostCenter as $customer)
                               @if ($customer->id == $invoice->CostCenter)
                                  <option selected value="{{$customer->id}}" >{{$customer->CostName}}</option>
                               @else
